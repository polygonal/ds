...
class TestHashMap extends AbstractTest
{
	function testRemove()
	{
		var h = new ds.HashMap<String, Null<Int>>();
		for (i in 0...2)
		{
			h.set("a", 1);
			h.set("b", 1);
	...
	function testGetSet()
	{
		...
		var h = new ds.HashMap<Int, Null<Int>>();
		h.set(1, 1);
		h.set(2, 2);
		h.set(3, 3);
		...
		var h = new ds.HashMap<Int, String>();
		h.set(1, "a");
		h.set(2, "b");
		h.set(3, "c");
	...
	function testClone()
	{
		...
		var clone:ds.HashMap<String, String> = cast h.clone(true);
	...
	function testIterator()
	{
		...
		var c:ds.Set<Int> = cast s.clone(true);
		var itr = h.iterator();
