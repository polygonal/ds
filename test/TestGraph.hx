...
class TestGraph extends AbstractTest
{
	...
	function testIterator()
	{
		...
		var s = new ds.HashSet<E>(1024);
		for (i in graph) assertEquals(true, s.set(i));
		assertEquals(10, s.size);
		
		var c:ds.Set<E> = cast s.clone(true);
		...
		var node:ds.GraphNode<E> = graph.getNodeList();
		while (node != null)
		{
			var node1:ds.GraphNode<E> = graph.getNodeList();
			while (node1 != null)
			{
				if (node != node1)
					node.addArc(node1, 1);
		...
		var s = new ds.IntHashSet(1024);
		for (i in graph) assertEquals(true, s.set(i));
		assertEquals(10, s.size);
		
		var node:ds.GraphNode<Int> = graph.getNodeList();
		while (node != null)
		{
			var node1:ds.GraphNode<Int> = graph.getNodeList();
			while (node1 != null)
			{
				if (node != node1)
					node.addArc(node1, 1);
...
private class E extends ds.HashableItem implements ds.Visitable
{
